...
export function TeacherPortal() {
  ...
  const conversations = data ? data.children.filter(child => child.guardianIds.length || data.messages.some(item => item.childId === child.id)) : [];
  const activeChild = conversations.find(child => child.id === conversationId) ?? conversations[0];
  const thread = data && activeChild ? data.messages.filter(item => item.childId === activeChild.id) : [];
  // Opening a conversation clears its unread badge everywhere.
  useMarkThreadRead(active === 'messages' ? thread : undefined, user.id);
  ...
  const notifications = data.messages
    .filter(item => item.senderId !== user.id && !item.readBy.includes(user.id))
